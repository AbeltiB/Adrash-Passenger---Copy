...
export class TripRepository { list(filters: TripFilters, signal?: AbortSignal) { return getPage<unknown>('/trips', { page: 1, pageSize: 20, status: 'Scheduled', ...filters }, signal).then((p) => ({ ...p, items: p.items.map(mapTrip) })); } detail(tripId: string, signal?: AbortSignal) { return getData<unknown>(`/trips/${tripId}`, undefined, signal).then(mapTrip); } seats(tripId: string, signal?: AbortSignal) { return getData<{ totalSeats: number; seats: SeatDTO[] }>(`/trips/${tripId}/seats`, undefined, signal).then((r) => r?.seats ?? []); } latestLocation(tripId: string, signal?: AbortSignal) { return getData<Record<string, unknown> | null>(`/trips/${tripId}/location/latest`, undefined, signal).then((r) => (r ? ({ lat: r.lat, lng: r.lng, heading: r.heading, speed: r.speed, timestamp: (r.timestamp ?? r.recordedAt), currentStopId: r.currentStopId } as TripLocationDTO) : (r as unknown as TripLocationDTO))); } }
